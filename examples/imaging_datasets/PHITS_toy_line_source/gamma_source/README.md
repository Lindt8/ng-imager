...
## Gamma-ray line source

_This is identical to the neutron line source example one directory higher, but with the source particles set to photons instead of neutrons._
...
The source is a "line source" of gamma rays with a Gaussian energy distribution centered at 6 MeV and with FWHM of 1 MeV (10^7 histories).  It is centered at (0, 3 cm, 4 cm) and oriented along the y-axis, spanning from y=-1 to y=7 cm (with a 0.4 cm radius).

![](source_YZ.png)

The produced `usrdef.out` file from this tally contains three-fold gamma-ray coincident event data.  This file is then processed by `ng-imager`, where the neutrons event cones are created and imaged.  
